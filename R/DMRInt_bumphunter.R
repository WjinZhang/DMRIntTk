...
#' @return The DMR set detected by bumphunter.
#' @export
#' @examples
...
DMRInt_bumphunter<-
  function(beta, pheno, arraytype=c("450K", "EPIC"), minProbes){
    #Check the installation of the ChAMP package
    if(require("ChAMP")){
    ...
    #Load the beta file and the selected arraytype
    arraytype = arraytype
    beta=as.matrix(beta)
    bumphunter <- champ.DMR(beta = beta,pheno=pd$Sample_Group, method="Bumphunter", arraytype = arraytype, minProbes = 3)
