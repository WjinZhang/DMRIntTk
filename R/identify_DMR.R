...
#'
#' ##For EPIC data:
#' beta = load(system.file("extdata", "beta_EPIC.RData", package = 'DMRIntTk'))
...
identify_DMR<-
  function(beta = beta, pheno = pd , method = c("bumphunter","combp","ipDMR","mCSEA","ProbeLasso","seqlm"),  arraytype = c("450K", "EPIC"), group1 = group1, group2 = group2, minProbes = minProbes,
    ...
    if(arraytype == "450K"){
      information <- DMRIntTk::data_450K
    }else{
      information <- DMRIntTk::data_EPIC
    }
    ...
    if("seqlm" %in% method){
      ...
      message("-----------------------------")
    if(arraytype == "450K"){
      Gr1<-DMRIntTk::Gr1_450K
    }else{
      Gr1<-DMRIntTk::Gr1_EPIC
    }
