...
#' @param totalDMR The DMR set (obtained from the function DMRInt_input function).
...
#'
#' @examples
#' interval_weight=DMRInt_weight(bin_method, totalDMR, pd, beta, group1, group2)
DMRInt_weight=function(bin, totalDMR, pd, beta, group1, group2){
  ...
  while(t < 1){
    ...
    calculate_DMRn <- function(totalDMR,count){
      message(sprintf("[%s] # Calculate the DMR score at all thresholds",
                      Sys.time()))
      totalDMR$start=as.numeric(totalDMR$start)
      totalDMR$end=as.numeric(totalDMR$end)
      ...
      for(n in c(1:10)){
        t1 <- (n-1)*(0.1^count)
        t2 <- n*(0.1^count)
        tmp <- totalDMR[which(abs(totalDMR$diff) > t1 & abs(totalDMR$diff) <= t2), ]
