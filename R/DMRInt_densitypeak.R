...
library(dplyr)
#Remain the column of chromosome positions and weight of the intervals as the DMRs ready for clustering
...
#Clustering was performed only for the first 4 thresholds of the center and neighbor thresholds
#(because as the thresholds increase, the number of DMRs decrease rapidly)

for(th_center in cenweight_par[1:4]){
  for(threshold in neiweight_par[1:4]){
    if(th_center>threshold){
   #Find local max intervals whose weights greater than the center threshold
      temp <- DMR
      ...
      #Determine the real locmax and assign other intervals to the closest locmax
      ...
      #Remove the misclassified centers whose weights are less than the center threshold
      #This misclassification is caused because it is the only interval on the chromosome
      fake_cen<-which(temp$local_max==-1 & temp$weight_diff<th_center)
      temp$local_max[fake_cen]=0


      #After determining clustering centers, extend each clustering center to both sides to include adjacent intervals
      #with thresholds greater than the neighbor threshold
      ...
      #Merge adjacent intervals as final DMRs
      ...
      if(dim(combined_dmr2)[1]>1){
        ...
        #Calculate the proportion of DMRs with methylation differences greater than threshold1 (i.e., 1/3 of the maximum difference)
        #and threshold 2 (2/1 of the maximum difference) in the integrated DMRs, respectively
        ...
        #Load 450K and EPIC probe annotation files to identify probes included in integrated DMRs
        ...
        #Generate integrated DMR sets under different center and neighbor thresholds
...
#Construct a matrix sin_pro to count the proportion of DMRs whose differences are greater
#than threshold1 and threshold2 in individual DMRs sets predicted by a single method
...
#Output the final integrated DMR set
