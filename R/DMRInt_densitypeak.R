#' Cluster all bins based on density peak algorithm.
#'
#' @param bin The bin with calculated weights(obtained from DMRInt_weight function).
#' @param input_DMR The DMR sets obtained from DMRIntTk_input function.
#'
...
DMRInt_densitypeak=function(bin,input_DMR,prefer=c("probe","length"), arraytype=c("450K", "EPIC")){
    library(dplyr)
    DMR<-dplyr::select(bin,c(chr,start,end,weight_diff,diff))
    ...
    maxdiff=max(abs(input_DMR$diff))
      ...
      for(threshold in neiweight_par[1:4]){
        if(th_center>threshold){
          ...
          if(dim(combined_dmr2)[1]>1){
            ...
            illumina450k <- DMRIntTk::data_450K
            illuminaEPIC <- DMRIntTk::data_EPIC
          ...
          }else{  #The case of only 1 bin left after clustering (it may occur when the center threshold is equal to 10/10)
    ...
    for(name in method){
      print(name)
      pro1=length(which(abs(input_DMR[input_DMR$methodname==name,]$diff)>threshold1))/length(which(input_DMR$methodname==name))
      pro2=length(which(abs(input_DMR[input_DMR$methodname==name,]$diff)>threshold2))/length(which(input_DMR$methodname==name))
